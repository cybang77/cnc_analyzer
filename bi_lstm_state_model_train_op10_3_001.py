...
# 모델 저장 서브 디렉토리명
MODEL_SUB_DIR_NAME = 'bi-lstm-state'
# 모델파일 접두사
MODEL_FILE_PREFIX = 'bi-lstm-state-model-'
# 학습데이터 루트 디렉토리명
...
def create_bi_state_lstm_model(look_back=1):
    ...
    for i in range(4):
        model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32, batch_input_shape=(1, look_back, 1), stateful=True, return_sequences=True)))
        model.add(tf.keras.layers.Dropout(0.3))

    model.add(tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, stateful=True, return_sequences=False)))
    model.add(tf.keras.layers.Dropout(0.3))
